...
;; Time-stamp: <2009-09-16 22:04:38 jjf>
...
;; you enable modal-mode, which results in a call to
;; modal-local-mode-enable for each buffer.  modal-local-mode ensures
;; that modal-global-mode is on.
;;
;; you disable modal-mode, which disables modal-local-mode in each
;; buffer.  modal-local-mode ensures that modal-global-mode is off, as
;; well as disables all modals in the current buffer.
...
(defvar modal-cmd-mode-map (make-sparse-keymap))
(mapc
 (lambda (x) (define-key modal-cmd-mode-map (eval (car x)) (cdr x)))
...
(add-to-list
 ...
 `(modal-cmd-mode . ,modal-cmd-mode-map))
